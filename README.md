...
```
embedded/
  __init__.py        공개 API 재수출 (create_model 등)
  base.py            BaseEmbeddedModel / DenseCapable / SparseCapable
  registry.py        이름→모델 팩토리 (register / register_preset / create_model)
  hf_utils.py        HF Hub 경로 해석 (resolve_model_path)
  models/
    __init__.py      구현체 import → 레지스트리 자동 등록
...
`create_model`로 바로 쓸 수 있는 것은 두 개다.

- `"bge-m3"` — dense + sparse (FlagEmbedding)
- `"sentence-transformer"` — 범용 dense 백엔드 (모델은 인자로 지정)

```python
from embedded import create_model, SparseCapable

model = create_model("bge-m3", model_name="BAAI/bge-m3")   # 또는 로컬 경로
...
구체 dense 모델은 패키지에 하드코딩하지 않는다. 필요한 모델을 사용자가 등록한다.
...
> 빠뜨려도 에러 없이 조용히 품질만 떨어지므로 등록 시 함께 지정할 것.
...
```python
from embedded import BaseEmbeddedModel, DenseCapable, register
...
with create_model("bge-m3", ...) as model:   # 자동 해제 (예외에도 안전)
...
```python
model = create_model("bge-m3", model_name="/srv/models/bge-m3")
```
